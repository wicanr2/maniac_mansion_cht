...
原版部分把 Enhanced DOS 版（SCUMM v2）完整繁體中文化：**1139 行文字、15 個指令、物件名、選角資料、片頭與結局全部中文**，字型用**倚天中文系統的 24×24 原生點陣字**，畫面拉到 640×456 讓它放得進去，透過 ScummVM 執行。

![指令列](screenshots/verbbar-zh.png)

15 個指令維持原版「5 欄 × 3 列」的排版，只是列距從 8 個邏輯像素放寬到 14——24×24 的倚天字等於 12 個邏輯像素高，照原本的 8 會上下咬在一起。上面那條洋紅色的句子列會把指令與物件名組成一句話：
...
字模用 24×24（等於 12 個邏輯像素），比原版 8×8 的英文大一半，筆劃多的字才擺得下。代價是指令區要重排：那裡要放六列文字（句子列 1、指令 3、物品欄 2），一列 14 就是 84 個邏輯像素，而原本的 144–200 只有 56。差額**不從房間或上方那條字幕帶挖**，而是把畫面總高從 200 加到 228（字幕帶 16 + 房間 128 + 指令區 84）。指令與物品欄的座標一起換算成 14 格制，那些矩形同時是點擊判定區，所以滑鼠命中會跟著走。
