...
**字形怎麼選的**：掃過 6 種字型 × size 12–14 × 二值化門檻 × 字距 × 基線跳動，
約 60 組逐張比對。判準不是肉眼「好不好看」，而是把「關」（19 畫）、「談」、「查」
印成 1× 的點陣圖，**逐格檢查筆畫之間有沒有留下背景像素**：

| 候選 | 「關」 |
|---|---|
| WQY 12px 點陣 strike | 門框與內部每一筆都隔一格背景，最乾淨 |
| **WQY outline 14px、門檻 100（採用）** | 有一列橫向連通，但仍可讀；字高與英文齊 |
| 華康超圓體 13px | 內部糊成一塊，只剩門框 |
| 華康少女文字／王漢宗波浪／海報體 | 結構崩壞，任何 size/門檻都不可用 |

採用 outline 14px 而不是更乾淨的 12px 點陣，是因為**要對上英文那種手繪不勻感**：
outline 光柵化下直筆 2px、橫筆 1px，點陣 strike 太工整反而做不出來；14px 也讓字高
跟英文一樣填滿按鈕（12px 明顯小一號）。另外加了字距 10（原版英文把按鈕填到 97%，
...
驗證：建置端與玩家端產出的 `.spr` md5 相同（`e5bd879f`），玩家的 `Maniac.exe` md5 不變；
