...
### 但 GUI 那邊要小一號：`agsfnt0` 縮到 0.7

16px 對白剛好，對**選單**卻太大：`F5` 叫出來的存檔／載入／開始／離開四個按鈕，框是固定尺寸的圖，16px 中文會撐出框外，四個標籤糊在一起。

先試了譯文裡那組 `font_320` / `font_640` 數字——俄文版就是靠改它們調位置的（英文 `13 14 14…`，俄文 `4 5 5…`）。實測**對我們沒有作用**，畫面完全沒變。

改用字型分槽解決，實測結論：

| 字型槽 | 用在哪 | 我們給的尺寸 |
|---|---|---|
| `agsfnt0` | GUI 標籤（選單按鈕）與畫面下方的句子列 | **0.7 倍**（`--scale 0.7`，改 unitsPerEm） |
| `agsfnt1` 以上 | 角色對白、人物簡介 | 原尺寸 |

`--scale` 改的是 `unitsPerEm`，只縮字形不縮名目高度；單行標籤本來就不受行距影響，所以這裡用它剛剛好——正是前面那條「行距不會跟著縮」的性質，在這個用途反而是優點。改完之後四個按鈕各自落在框裡，句子列也不再擠到 USE 按鈕。
