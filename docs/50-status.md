...
| 畫面 | 640×456（hi-res 文字表面 + 指令區加高） | 640×400（`acsetup.cfg` 的 `[override] upscale=1`，純資料） |
...
驗證：選角畫面的人物簡介、句子列、指令列三列不重疊、點「查看」會正確反白（點擊判定跟著新格線走）。
**還沒視覺確認**：24×24 下的多行對白折行、物品欄有東西時的樣子。
