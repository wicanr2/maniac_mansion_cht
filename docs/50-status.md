...
## v2 用 24×24 字模：做過了，卡在指令列

「中文能不能再大一點」的實驗結果。倚天 24 點（`STD.24M`，ETUNPACK 解壓後同一套 Big5 分區索引）
烘成同格式的 `chinese_gb16x12.fnt`，引擎那邊只多兩行：字模尺寸改成**由字型檔大小決定**
（`numChar × 30` = 16×15、`× 72` = 24×24），行距改成 `_2byteHeight / _textSurfaceMultiplier`。

| | 16×15（現行） | 24×24 |
|---|---|---|
| 邏輯尺寸 | 8 × 7.5 | 12 × 12 |
| 對白 | 一行 40 字 | 一行 26 字，筆劃清楚很多 |
| 指令列（3 列 × 8 邏輯像素） | ✅ 剛好 | ❌ 每列往下吃 4px，上下咬在一起 |

「把列距從 8 改成 12」這條路**算過之後行不通**。v2 的畫面是房間 144px + 指令區 56px = 200px，
指令區裡有 6 列文字：

| 列 | 相對 y（8px 格） | 改 12px 格 |
|---|---|---|
| 句子列 | 0 | 0 |
| 指令 3 列 | 8 / 16 / 24 | 12 / 24 / 36 |
| 物品欄 2 列 | 32 / 40 | 48 / 60 |
| 合計 | 48 ≤ 56 ✅ | **72 > 56 ❌** |

6 × 12 = 72，指令區只有 56。除非把房間區壓小（動到所有背景與角色座標，等於重做版面），
不然 24×24 沒有位置放。

剩下的路是**混用**：對白（畫在房間區）用 24×24，指令區維持 16×15。引擎在 `printChar()`
已經知道自己畫在哪個虛擬螢幕，所以技術上做得到——要同時載入兩份字模，
並在 `drawString()` 依 `ypos` 切換 `_2byteWidth/_2byteHeight/_2byteFontPtr`，
換行計算也要跟著切。大約 30 行，已超出「最小修補」，先停在這裡等決定。
字型與尺寸偵測都已經可用，`game-cht24/` 換上 24×24 的字型檔就能重現。
