...
指令列的列距是遊戲腳本寫死的 8 像素。要用 24×24 就得**在引擎裡把 verb 的 y 座標從 8 的格子
重新映射到 12 的格子**（那塊區域有 56 個邏輯像素，4 列 × 12 = 48 塞得下），點擊判定會跟著一起走——
但這已經超出「最小修補」，所以先停在這裡等決定。字型與引擎的部分都已經可用，
`game-cht24/` 換上 24×24 的字型檔就能重現。
